...
export class ProductEffect {
  ...
  @Effect()
  searchProducts$: Observable<Action> = this.actions$.pipe(
    ofType<productActions.SearchProducts>(
      productActions.ProductActionTypes.SEARCH_PRODUCTS
    ),
    map((action: productActions.SearchProducts) => action.payload),
    mergeMap((searchProduct: SearchProduct) =>
      this.productService.searchProducts(searchProduct).pipe(
        map(
          (products: Product[]) =>
            new productActions.SearchProductsSuccess(products)
        ),
        catchError(err => of(new productActions.LoadProductsFail(err)))
      )
    )
